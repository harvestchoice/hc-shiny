...
i <- 1L                                         # number of commodities
r <- 2L                                         # number of regions
t <- 10L                                        # time periods
...
dream <- function(sets, vars, params) {
  ...
  k_max <- p * a_max * PP[,,1] * ((Y / epsilon_shift[,,1]) - (c / (1 + y)))    # 2.8
  ...
  adoptT <- function(x, k_max, lagR, lagA, lagM, lagD) {
    ...
    # Segments
    y <- sapply(1:x, function(i) ifelse( i>lagR & i<=lagR+lagA, k_max*(i-lagR)/lagA,
      ifelse( i>lagR+lagA & i>=lagR+lagA+lagM, k_max,
        ifelse( i<lagR+lagA+lagM & i>=lagR+lagA+lagM+lagD, k_max*(lagR+lagA+lagM+lagD-i)/lagD,
          array(0, dim=c(i,r,t))))), simplify="array")
    dimnames(y) <- sets
    return(y)
  }
