...
renderRHandsontable(
  rhandsontable(vi.sum, width=540, stretchH="last",
...
vi.miss <- vi[published==T, lapply(.SD, function(x) sum(is.na(x), na.rm=T)),
...
renderRHandsontable(
  rhandsontable(vi.miss, rowHeaders=F, readOnly=T, fixedColumnsLeft=3, contextMenu=F) %>%
...
selectInput("cat", "Select a category", cat)
...
output$hot <- renderRHandsontable({
  ...
  values$hot <- df
  rhandsontable(df, readOnly=T, rowHeaders=F, fixedColumnsLeft=1, width=860, stretchH="all",
    highlightCol=T, highlightRow=T, columnSorting=T)
})
