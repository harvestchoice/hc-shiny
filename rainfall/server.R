    ...
    output$dygraph <- renderDygraph({
        ...
        isolate ({
            ...
            dygraph(xts::as.xts(dt[, list(value, mean, meanAnnual, trend)], order.by=dt$month)) %>%
              if (input$selectSeries==1) dySeries("value", label=var()) %>%
              if (input$selectSeries==2) dySeries("meanAnnual", label="annual mean", fillGraph=F, strokeWidth=2) %>%
              if (input$selectSeries==3) dySeries("mean", label="period mean") %>%
              if (input$selectSeries==4) dySeries("trend", label="trend", fillGraph=F, strokeWidth=3, strokePattern="dashed") %>%
